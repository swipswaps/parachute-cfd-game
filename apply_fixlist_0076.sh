...
# ----------------------------------------------------------------------
# apply_fixlist_0076.sh – Complete self‑healing CFD‑to‑Godot pipeline
# v76 : automated Godot scene + interactive launch
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self‑Healing v76 ==="
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v76 automated Godot scene + interactive launch'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

# *** NEW v76 step: interactive Godot launch ***
launch_godot_step

llm_audit_info
echo ""
echo "✅ All fixes applied! (v76)"