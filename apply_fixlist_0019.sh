...
# ----------------------------------------------------------------------
# apply_fixlist_0019.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Installs all system dependencies (OpenFOAM, ParaView, Godot, GDAL)
# - Installs all Python packages (vtk, trimesh, pycollada, fast_simplification, etc.)
# - Creates all OpenFOAM template files (0.orig, system/, constant/)
# - Creates stub scripts, populates docs, verifies collada_to_stl with a cube
# - Auto‑downloads terrain if no DAE provided (via OpenTopography + OSM)
# - Runs the full CFD mesh, simulation, wind extraction, and Godot asset copy
# - All steps are idempotent (check → apply → verify)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# OpenFOAM repo (already added by v12, but check)
...
run_step "ParaView (pvpython)" \
    "command -v pvpython || find /usr -name pvpython | grep -q pvpython" \
    "sudo dnf install -y paraview && PVPY=$(find /usr -name pvpython | head -1) && [ -n \"$PVPY\" ] && sudo ln -sf \"$PVPY\" /usr/local/bin/pvpython" \
...
# GDAL development headers (for Python GDAL)
run_step "GDAL devel" \
    "gdal-config --version" \
...
# ----------------------------------------------------------------------
# 2. Python packages (including terrain and CFD tools)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 3. OpenFOAM template files (0.orig, system, constant) – idempotent content checks
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 5. Test collada_to_stl with a cube (skip simplification)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 7. Create terrain helper script (for auto‑download)
# ----------------------------------------------------------------------
