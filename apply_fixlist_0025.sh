...
# ----------------------------------------------------------------------
# apply_fixlist_0025.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Fixed ParaView step: no run_step, uses plain idempotent block to avoid variable expansion.
# - All other steps identical to v0024.
# ----------------------------------------------------------------------
...
# Helper: idempotent step runner (used for all other steps)
run_step() {
    local step_name="$1"
    local check_cmd="$2"
    local apply_cmd="$3"
...
# ----------------------------------------------------------------------
# ParaView – idempotent block (manual, avoids run_step variable expansion)
# ----------------------------------------------------------------------
...
if command -v pvpython &>/dev/null || find /usr -name pvpython 2>/dev/null | grep -q pvpython; then
    ...
    # Create temporary script using a quoted heredoc – $PVPY will NOT be expanded here.
