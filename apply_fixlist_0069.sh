...
# ----------------------------------------------------------------------
# apply_fixlist_0069.sh – Complete self-healing CFD-to-Godot pipeline
#
...
REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self-Healing v69 ==="
...
run_sim_step() {
    ...
    /Solving for Uy/ { match($0,/Initial residual = ([0-9.eE+\-]+)/,a); uy=a[1]+0 }
    /Solving for Uz/ { match($0,/Initial residual = ([0-9.eE+\-]+)/,a); uz=a[1]+0 }
    /Solving for p/  { match($0,/Initial residual = ([0-9.eE+\-]+)/,a); pr=a[1]+0 }
    /^ExecutionTime/ {
        iter++
        status = "running"
        if (ux<1e-4 && uy<1e-4 && uz<1e-4 && pr<1e-4) status = "CONVERGED"
        if (ux>1 || uy>1 || uz>1 || ux!=ux)            status = "DIVERGED"
        if (ux >= prev*0.999) stall++; else stall=0
        if (stall>=50)                                  status = "STALLED"
        prev = ux
        # Every 10 iters or status change: persistent numbered line
    ...
    echo "  ✅ $solver done — latest time: $(ls -d [0-9]* 2>/dev/null | sort -n | tail -1)"
    cd - >/dev/null
}
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v69 collapsed numbered sim output, stall/diverge/converge detect, log dump on failure, self-heal,, no blocking loops, stub fix, OSM terrain pipeline, GE export removed, OSM2World fixes'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

echo ""
echo "✅ All fixes applied! (v69)"
echo ""
echo "Next steps:"
