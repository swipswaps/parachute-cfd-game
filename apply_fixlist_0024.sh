...
# ----------------------------------------------------------------------
# apply_fixlist_0024.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Adds missing Flatpak setup, raster2mesh, osm2world, blender.
# - Fixed ParaView step: uses /tmp/pvpy_setup.sh.
# - All steps idempotent (check → apply → verify).
# ----------------------------------------------------------------------
...
# ParaView – fixed: use fixed temporary script path
...
EOF
chmod +x /tmp/pvpy_setup.sh && sudo dnf install -y paraview && /tmp/pvpy_setup.sh && rm -f /tmp/pvpy_setup.sh" \
    "pvpython --version 2>&1 | grep -qi paraview"

# Flatpak setup (install flatpak and add flathub remote)
...
# ----------------------------------------------------------------------
# 2. Python packages (including terrain tools)
# ----------------------------------------------------------------------
run_step "Python packages (vtk, trimesh, pycollada, fast_simplification, raster2mesh, etc.)" \
    "python3 -c 'import trimesh, vtk, pykml, lxml, osgeo, scipy, matplotlib, collada, fast_simplification, raster2mesh' 2>/dev/null" \
    "python3 -m pip install --user --break-system-packages numpy vtk trimesh pykml lxml GDAL scipy matplotlib pycollada fast_simplification raster2mesh" \
    "python3 -c 'import trimesh, vtk, pykml, lxml, osgeo, scipy, matplotlib, collada, fast_simplification, raster2mesh; print(\"OK\")'"

# ----------------------------------------------------------------------
# 3. Terrain generation tools (osm2world, blender)
# ----------------------------------------------------------------------
run_step "osm2world" \
    "command -v osm2world" \
    "wget -O /tmp/osm2world.jar https://github.com/tordanik/OSM2World/releases/download/0.2.0/OSM2World-0.2.0.jar && echo 'java -jar /tmp/osm2world.jar \"\$@\"' | sudo tee /usr/local/bin/osm2world && sudo chmod +x /usr/local/bin/osm2world" \
    "command -v osm2world"
...
# ----------------------------------------------------------------------
# 8. Create terrain helper script (for auto‑download)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 9. User input and terrain auto‑download (if needed)
# ----------------------------------------------------------------------
