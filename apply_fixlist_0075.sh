...
# ----------------------------------------------------------------------
# apply_fixlist_0075.sh – Complete self‑healing CFD‑to‑Godot pipeline
# v75 : automated Godot scene setup (terrain + collision)
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self‑Healing v75 ==="
...
'FoamFile\n{ version 2.0; format ascii; class dictionary; object turbulenceProperties; }\nsimulationType RAS;\nRAS { RASModel kEpsilon; turbulence on; printCoeffs on; }\n')"
    ...
    if grep -q "cannot find file.*/0/nut" "$log" 2>/dev/null; then
        ...
        mkdir -p "$case_dir/0"
        python3 -c "..."   # full nut field content
        echo "  ✅ 0/nut written"
        return 0
    fi
    # OMEGA-MISSING
    if grep -q "cannot find file.*/0/omega" "$log" 2>/dev/null; then
        ...
        mkdir -p "$case_dir/0"
        python3 -c "..."   # full omega field content
    ...
    if [[ -n "$missing_field" ]]; then
        ...
        mkdir -p "$case_dir/0"
        python3 -c "..."   # placeholder
        echo "  ✅ 0/$missing_field written"
        return 0
    fi
...
setup_godot_scene() {
    ...
    run_step "Configure Godot scene (terrain + collision)" \
        "test -f '$scene_file' && grep -q 'ConcavePolygonShape3D' '$scene_file'" \
        "
        mkdir -p godot_project/scenes
        if [ ! -f '$scene_file' ]; then
            cat > '$scene_file' << 'SCENEHEAD'
[gd_scene load_steps=4 format=3]
[ext_resource type=\"PackedScene\" path=\"res://.godot/imported/terrain_fallback.glb\" id=\"1_geometry\"]
[sub_resource type=\"ConcavePolygonShape3D\" id=\"ConcavePolygonShape3D_terrain\"]
data = PackedVector3Array()
[sub_resource type=\"BoxShape3D\" id=\"BoxShape3D_placeholder\"]
size = Vector3(1, 1, 1)
SCENEHEAD
        fi
        export STL_PATH='$stl_path' SCENE_FILE='$scene_file'
        python3 - << 'PYEOF'
import os, sys
stl_file = os.environ['STL_PATH']
...
mesh = SubResource(\"{stl_file}\")
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0)

[node name=\"StaticBody3D\" type=\"StaticBody3D\" parent=\"./{name}\"]
transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0)

[node name=\"CollisionShape3D\" type=\"CollisionShape3D\" parent=\"./{name}/StaticBody3D\"]
shape = SubResource(\"ConcavePolygonShape3D_terrain\")
'''
with open(scene_file, 'a') as f:
    f.write(terrain_node)
print('Scene updated with terrain and collision.')
PYEOF
        " \
        "test -f '$scene_file' && grep -q 'ConcavePolygonShape3D' '$scene_file'"
}
...
# *** NEW v75 step: automate Godot scene setup ***
setup_godot_scene
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v75 automated Godot scene with terrain + collision'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

llm_audit_info
echo ""
echo "✅ All fixes applied! (v75)"
echo ""
echo "Next step:"
echo "  godot godot_project/project.godot   # scene is ready, just press F5"