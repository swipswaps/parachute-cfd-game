...
# ----------------------------------------------------------------------
# apply_fixlist.sh – executes prf.fixlist.v6 on Fedora 43
# Run from the root of the repository (where README.md and scripts/ exist)
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "Applying fixes in: $REPO_ROOT"

# Helper: run a step with its check, apply, verify
run_step() {
    local step_name="$1"
    local check_cmd="$2"
    local apply_cmd="$3"
...
# ----------------------------------------------------------------------
# ENV-001 – source OpenFOAM env in .bashrc (idempotent)
...
# ----------------------------------------------------------------------
# TEST-001 – verify collada_to_stl with a cube
