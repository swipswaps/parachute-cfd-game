...
# ----------------------------------------------------------------------
# apply_fixlist_0041.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Fixes Google Earth Pro: never launches second instance; raises existing window.
# - Eliminates "Another instance is already running" popup.
# - Full OpenFOAM templates, CFD pipeline, and Godot asset copy included.
# ----------------------------------------------------------------------
...
# Ensure xdotool is installed (for window raising)
run_step "xdotool" \
    "command -v xdotool" \
...
# ----------------------------------------------------------------------
# 8. User input (raises existing Google Earth Pro window, never launches second)
# ----------------------------------------------------------------------
...
if [[ -z "${LOCATION_NAME:-}" ]]; then
    ...
    # Raise existing Google Earth Pro window or launch fresh (only once)
    if check_google_earth; then
        read -r -p "Open Google Earth Pro? (y/n, default y): " open_ge
        open_ge=${open_ge:-y}
        if [[ "$open_ge" =~ ^[Yy]$ ]]; then
            if pgrep -f "google-earth-pro" >/dev/null; then
                echo "Google Earth Pro already running. Bringing existing window to front..."
                # Try to raise by class
                WINDOW=$(xdotool search --class "google-earth-pro" 2>/dev/null | head -1)
                if [[ -n "$WINDOW" ]]; then
                    xdotool windowactivate "$WINDOW" 2>/dev/null
                else
                    # Fallback: raise by name
                    WINDOW=$(xdotool search --name "Google Earth" 2>/dev/null | head -1)
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (raise GE window, never second instance)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
