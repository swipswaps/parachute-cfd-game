...
static func inverse(a: Array) -> Array:
	...
	var idx = identity_matrix(n)
	var im = I.duplicate(true)

	for fd in range(n):
		var fd_scaler: float = 1.0 / am[fd][fd]
...
static func ray_plane_intersection(
	p1: Vector3, n1: Vector3, p2: Vector3, n2: Vector3, p0: Vector3
) -> Array[Vector3]:
	var m := [
		[2, 0, 0, n1.x, n2.x],
		[0, 2, 0, n1.y, n2.y],
	...
	var b = [[2 * p0.x][2 * p0.y][2 * p0.z][b4][b5]]
...
static func zero_matrix(n_x: int, n_y: int) -> Array:
	var matrix := []
	for x in range(n_x):
		matrix.append([])
# warning-ignore:unused_variable
