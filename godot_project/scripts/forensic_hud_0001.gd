...
func _gui_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			dragging = true
			drag_start = get_global_mouse_position() - global_position
		else:
			dragging = false
		accept_event()
	elif event is InputEventMouseMotion and dragging:
		global_position = get_global_mouse_position() - drag_start
		print("[ForensicHUD] Drag moved to: ", global_position)
...
# are HTTP clients of the same forensic_hub_gamified.py server; single
# source of truth (Design by Contract, Meyer 1988 ISBN 978-0-13-629155-8).
#
# Godot API references (all URLs confirmed via search in this session):
...
#   OS.shell_open    https://docs.godotengine.org/en/stable/classes/class_os.html#class-os-method-shell-open
#
# WHY this rewrite (Root Cause Analysis, 5 Whys):
#   The prior on-disk forensic_hud.gd extended CanvasLayer and called
#   get_global_mouse_position() -- defined on CanvasItem, not CanvasLayer,
#   so gdparse rejected it (transcript lines 17554-17560). This file
#   extends Node, builds its own CanvasLayer child, and uses event.position
#   from InputEventMouse{Button,Motion}, which is available on every event.
#
# Controls: F3 toggles the panel. Left-click + drag anywhere on the panel
# to move it. Each citation card has a [+]/[-] toggle. Clicking a URL row
# opens it in the system browser via OS.shell_open.
# ==========================================================================
...
var _card_expanded: Dictionary = {}  # id -> bool
...
func _handle_mouse(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		...
		if mb.button_index != MOUSE_BUTTON_LEFT:
			return
		var rect := Rect2(_panel.position, _panel.size if _panel.size != Vector2.ZERO else PANEL_MIN_SIZE)
	...
	if event is InputEventMouseMotion and _dragging:
		...
		_panel.position = mm.position - _drag_offset
		get_viewport().set_input_as_handled()
...
func _build_ui() -> void:
	...
	_panel.position = PANEL_START_POS
...
func _ready() -> void:
	...
	# Enable selection on any RichTextLabel children
	for child in get_children():
		if child is RichTextLabel:
			child.selection_enabled = true
	# Also find nested children if needed
...
func _poll_slow() -> void:
	# vFHP1.1: guard against concurrent HTTPRequest calls.
	# get_http_client_status() returns STATUS_DISCONNECTED(0) when idle.
	# vFHP1.1: guard against concurrent HTTPRequest calls.
	# get_http_client_status() returns STATUS_DISCONNECTED(0) when idle.
	if http_request and http_request.get_http_client_status() != HTTPClient.STATUS_DISCONNECTED:
		print("[FPANEL] _poll_slow: request in flight, skipping")
		return
	if http_request and http_request.get_http_client_status() != HTTPClient.STATUS_DISCONNECTED:
		print("[FPANEL] _poll_slow: request in flight, skipping")
		return
...
func _on_leader_completed(result: int, response_code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	if result != HTTPRequest.RESULT_SUCCESS or response_code != 200:
		return
	var parsed = JSON.parse_string(body.get_string_from_utf8())
...
func _on_cite_completed(result: int, response_code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	if result != HTTPRequest.RESULT_SUCCESS or response_code != 200:
		return
	var parsed = JSON.parse_string(body.get_string_from_utf8())
	if typeof(parsed) != TYPE_DICTIONARY:
...
func _on_integ_completed(result: int, response_code: int, _headers: PackedStringArray, body: PackedByteArray) -> void:
	if result != HTTPRequest.RESULT_SUCCESS or response_code != 200:
		return
	var parsed = JSON.parse_string(body.get_string_from_utf8())
	if typeof(parsed) != TYPE_DICTIONARY:
