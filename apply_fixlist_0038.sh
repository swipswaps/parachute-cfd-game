...
# ----------------------------------------------------------------------
# apply_fixlist_0038.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Fixes Google Earth Pro launch: raises existing window instead of second instance.
# - Installs wmctrl if missing.
...
# ----------------------------------------------------------------------
# 8. User input (with Google Earth raise-or-launch logic)
# ----------------------------------------------------------------------
...
if [[ -z "${LOCATION_NAME:-}" ]]; then
    ...
    # Launch or raise Google Earth Pro
    if check_google_earth; then
        ...
        if [[ "$launch_ge" =~ ^[Yy]$ ]]; then
            if pgrep -f "google-earth-pro" >/dev/null; then
                ...
                wmctrl -a "Google Earth" 2>/dev/null || xdotool search --name "Google Earth" windowactivate 2>/dev/null
            else
                google-earth-pro &
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (fix GE single instance)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
