...
# ----------------------------------------------------------------------
# apply_fixlist_0068.sh – Complete self-healing CFD-to-Godot pipeline
#
...
REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self-Healing v68 ==="
...
# ─────────────────────────────────────────────────────────────────────
# run_sim_step: runs simpleFoam with live residual output + log dump
# ─────────────────────────────────────────────────────────────────────
run_sim_step() {
    local case_dir="$1"
    ...
    echo "  🔧 $solver (watching residuals — convergence target 1e-4)..."
    # SIM-GREP-HANG fix: no grep filter — tee direct to terminal verbatim.
    # Piping through grep caused buffering stall on fast failures.
    # Show everything. "Even if messy" — verbatim is correct.
    # stdbuf -oL: line-buffered tee — immediate output, no 64KB stall
    # || true: prevents set -e killing script on solver failure before diagnose_and_heal runs
    { $solver 2>&1 | stdbuf -oL tee log.$solver; } || true

    if grep -qE 'FOAM FATAL|Segmentation|Error' log.$solver 2>/dev/null; then
        echo "  ❌ $solver failed. Last 40 lines:"
        tail -40 log.$solver
        echo ""
        if diagnose_and_heal "log.$solver" "."; then
            ...
            if [[ -d 1000 ]]; then
                ...
                echo "  ❌ $solver still failed after heal"
                tail -20 log.${solver}.retry
                cd - >/dev/null; exit 1
            fi
        else
            cd - >/dev/null; exit 1
    ...
    if [[ ! -d 1000 ]]; then
        echo "  ❌ $solver did not produce time directory 1000"
        echo "  Last 40 lines of log.$solver:"
        tail -40 log.$solver
        cd - >/dev/null; exit 1
    fi
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v68 nut field, pipefail, stdbuf tee, heal rules, log dump on failure, self-heal,, no blocking loops, stub fix, OSM terrain pipeline, GE export removed, OSM2World fixes'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

echo ""
echo "✅ All fixes applied! (v68)"
echo ""
echo "Next steps:"
