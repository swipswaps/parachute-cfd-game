...
# ----------------------------------------------------------------------
# apply_fixlist_0056.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - v56: Force clean RPM extraction, verify executable binary.
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self‑Healing v56 ==="
...
# --- FIXED: Force clean extraction, verify executable ---
run_step "Extract Google Earth Pro RPM" \
    "test -x /tmp/ge_extract/opt/google/earth/pro/google-earth-pro" \
    "sudo rm -rf /tmp/ge_extract && mkdir -p /tmp/ge_extract && cd /tmp/ge_extract && rpm2cpio /tmp/google-earth-pro-stable.rpm | cpio -idmv 2>&1 | tail -3" \
    "test -x /tmp/ge_extract/opt/google/earth/pro/google-earth-pro"
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: v56 force clean GE extraction')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

echo ""
echo "✅ All fixes applied successfully! (v56)"
echo ""
echo "Next