...
# ----------------------------------------------------------------------
# apply_fixlist_0027.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Removes broken raster2mesh auto‑download feature.
# - User must provide a COLLADA (.dae) file from Google Earth Pro.
# - All other steps identical to v0026.
# ----------------------------------------------------------------------
...
# Git (still useful, but not required for auto-download now)
run_step "Install git" \
    "command -v git" \
...
# ----------------------------------------------------------------------
# 3. Terrain generation tools (osm2world, blender – kept but may be unused)
# ----------------------------------------------------------------------
run_step "osm2world" \
    "command -v osm2world" \
...
# ----------------------------------------------------------------------
# 8. Create terrain helper script (now disabled with error message)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 9. User input and terrain auto‑download (disabled – require DAE)
# ----------------------------------------------------------------------
...
if [[ -z "${LOCATION_NAME:-}" ]]; then
    ...
    # Force user to provide a DAE file; auto-download disabled
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (auto-download removed)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
