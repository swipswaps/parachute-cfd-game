...
# ----------------------------------------------------------------------
# apply_fixlist_0043.sh – Complete self‑healing CFD‑to‑Godot pipeline
...
# - Fixed missing verify argument in repo step.
# ----------------------------------------------------------------------
...
run_step "Google Earth Pro RPM" \
    ...
    "sudo dnf install -y google-earth-pro-stable" \
    "command -v google-earth-pro"
...
# ----------------------------------------------------------------------
# 3. OpenFOAM template files (full set – same as v41)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 4. Stub scripts and documentation (same as v41)
# ----------------------------------------------------------------------
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (RPM Google Earth, robust raise/kill)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
