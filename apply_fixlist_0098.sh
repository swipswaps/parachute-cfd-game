...
# ----------------------------------------------------------------------
# apply_fixlist_0098.sh – Complete self‑healing CFD‑to‑Godot pipeline v98
# v98 : adds Python dependency checks, ParaView flatpak wrapper, default Allrun.mesh
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Self‑Healing v98 (complete) ==="
...
dump_system_info() {
    ...
    echo "📝 Writing system dump: $dump_file"
    {
        ...
        python3 -c "import vtk, numpy, trimesh; print('vtk, numpy, trimesh OK')" 2>/dev/null || echo "Missing one or more"
        ...
        echo "=== Recent Logs ==="
        for log in *.log* 2>/dev/null; do echo "--- $log ---"; tail -20 "$log" 2>/dev/null || echo "Empty"; done
        echo "=== Linter Status ==="
...
check_python_dependencies() {
    ...
    local missing=()
    if ! python3 -c "import vtk" 2>/dev/null; then missing+=("vtk"); fi
    if ! python3 -c "import numpy" 2>/dev/null; then missing+=("numpy"); fi
    if ! python3 -c "import trimesh" 2>/dev/null; then missing+=("trimesh"); fi
    if [ ${#missing[@]} -gt 0 ]; then
        echo "⚠️ Missing Python packages: ${missing[*]}"
        echo "   Please install them via:"
        echo "   pip install ${missing[*]}"
        echo "   Or using your system package manager."
        read -p "   Attempt to install now with pip? (y/n) " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            pip install "${missing[@]}" || {
            ...
            echo "❌ Cannot proceed without required Python packages."
            exit 1
        fi
    else
...
# --------------- Auto‑detect and setup environment for all components ---------------
setup_environment() {
...
main() {
    ...
    # Create default Allrun.mesh if missing
    ensure_allrun_mesh

    # Check Python dependencies (vtk, numpy, trimesh)
    check_python_dependencies

    # Dump system info for LLM debugging
    dump_system_info
    ...
    launch_godot_step
    perform_llm_audit
    echo "=== All steps completed successfully (v98) ==="
}
