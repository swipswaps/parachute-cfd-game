...
# ----------------------------------------------------------------------
# apply_fixlist_0059.sh – Complete self-healing CFD-to-Godot pipeline
#
# v59 changes — all derived from confirmed evidence, not guesses:
#
...
REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self-Healing v59 ==="
...
run_step "Stub download_terrain_tiles.py" \
    ...
    "python3 -c \"open('scripts/download_terrain_tiles.py','w').write('#!/usr/bin/env python3\nimport sys\nprint(\\\"Use OSM pipeline in apply_fixlist_0059.sh instead.\\\")\nsys.exit(1)\n'); import os; os.chmod('scripts/download_terrain_tiles.py',0o755)\"" \
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v59 OSM terrain pipeline, GE export removed, OSM2World fixes'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
