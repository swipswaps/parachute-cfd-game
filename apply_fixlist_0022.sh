...
# ----------------------------------------------------------------------
# apply_fixlist_0022.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Fixed ParaView step using a temporary script to avoid unbound variable.
# - All other steps identical to v0021 (full template creation, terrain auto-download, etc.)
# ----------------------------------------------------------------------
...
# ParaView – fixed using a temporary script to avoid variable expansion issues
run_step "ParaView (pvpython)" \
    "command -v pvpython || find /usr -name pvpython 2>/dev/null | grep -q pvpython" \
    "TMP_SCRIPT=$(mktemp) && cat > $TMP_SCRIPT << 'EOF'
#!/bin/sh
...
EOF
chmod +x $TMP_SCRIPT && sudo dnf install -y paraview && $TMP_SCRIPT && rm -f $TMP_SCRIPT" \
...
# ----------------------------------------------------------------------
# 3. OpenFOAM template files (full set – same as v0021, but included here)
# ----------------------------------------------------------------------
