...
# ----------------------------------------------------------------------
# apply_fixlist_0086.sh – Complete self‑healing CFD‑to‑Godot pipeline v86
# v86 : fully restored script body (no placeholders), silent heal‑DB,
#       forced scene regeneration, auto‑audit
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self‑Healing v86 ==="
...
perform_llm_audit() {
    ...
    cp apply_fixlist_0086.sh "${audit_dir}/"
    ...
    echo "  ${audit_dir}/apply_fixlist_0086.sh"
...
# v86: Convert STL to OBJ, copy to Godot, generate scenes (forced if stale)
convert_stl_to_obj_and_copy
generate_terrain_scene
fix_main_scene
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v86 restored script body, forced scene regen, silent heal-DB, auto audit'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

perform_llm_audit
launch_godot_step
echo ""
echo "✅ All fixes applied! (v86)"