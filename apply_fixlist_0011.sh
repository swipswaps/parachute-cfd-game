...
# ----------------------------------------------------------------------
# apply_fixlist_0011.sh – Fully compliant with prf.fixlist.v6
# - Patches collada_to_stl.py to use target_reduction instead of target_faces.
# - Adds fast_simplification and pycollada.
...
# ----------------------------------------------------------------------
# PY-001 – Python packages (including fast_simplification)
...
# ----------------------------------------------------------------------
# PATCH-001 – fix collada_to_stl.py to use target_reduction instead of target_faces
run_step "PATCH-001 (fix_collada_to_stl_simplify)" \
    "grep -q 'target_reduction' scripts/collada_to_stl.py" \
    "sed -i 's/mesh = mesh.simplify_quadric_decimation(target_faces)/mesh = mesh.simplify_quadric_decimation(target_reduction = 1 - simplify_ratio)/' scripts/collada_to_stl.py" \
    "grep -q 'target_reduction' scripts/collada_to_stl.py"
...
# ----------------------------------------------------------------------
# TEST-001 – verify collada_to_stl with a cube (requires patched script)
...
run_step "GIT-001 (commit_all_fixes)" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply prf.fixlist.v6 — deterministic OpenFOAM templates, GDAL deps, alias, stubs, docs, pycollada, fast_simplification, patch collada_to_stl')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
