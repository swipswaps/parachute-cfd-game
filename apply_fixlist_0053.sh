...
# ----------------------------------------------------------------------
# apply_fixlist_0053.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Installs Google Earth Pro from RPM (no popups) using dnf localinstall.
# - Launches GE only once, caches DAE path.
...
# ----------------------------------------------------------------------
# 2. Install Google Earth Pro (RPM version – no popups)
# ----------------------------------------------------------------------
...
# Kill any stuck rpm process
sudo pkill -9 rpm 2>/dev/null || true

# Remove any existing RPM package
if rpm -q google-earth-pro-stable 2>/dev/null; then
    echo "Removing old RPM version..."
...
# Install with dnf localinstall (bypasses digest errors and respects transaction lock)
run_step "Install Google Earth Pro via dnf" \
    "command -v google-earth-pro" \
    "sudo dnf install -y --nogpgcheck /tmp/google-earth-pro-stable.rpm && rm -f /tmp/google-earth-pro-stable.rpm" \
    "command -v google-earth-pro"

# Symlink
sudo ln -sf /usr/bin/google-earth-pro /usr/local/bin/google-earth-pro

# xdotool (still useful for focus)
run_step "xdotool" \
    "command -v xdotool" \
...
# ----------------------------------------------------------------------
# 4. OpenFOAM template files (full set – same as v52)
# ----------------------------------------------------------------------
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (reliable RPM Google Earth)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
