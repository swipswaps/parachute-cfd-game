#!/usr/bin/env bash
set -eu
# pipefail temporarily disabled for process substitutions; re‑enabled after critical sections
trap 'echo " Interrupted by user"; exit 130' INT

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Self‑Healing v100 (complete) ==="
...
check_permissions() {
    ...
    for dir in "${dirs[@]}"; do
        if [ -d "$dir" ]; then
            ...
            # Directory doesn't exist yet; we will create later.
...
# --------------- heal database (full pattern set from v99) ---------------
...
diagnose_and_heal() {
    ...
    local fix_applied=0
    # Fetch patterns – may fail if no rows, but we handle gracefully
...
extract_wind_field_step() {
    ...
    # Use the virtual environment's Python for extraction
...
main() {
    ...
    # Auto‑setup environment (adds to PATH, sources OpenFOAM)
    setup_environment
    ...
    launch_godot_step
    perform_llm_audit
    echo "=== All steps completed successfully (v100) ==="
}
