...
# ----------------------------------------------------------------------
# apply_fixlist_0067.sh – Complete self-healing CFD-to-Godot pipeline
#
...
REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self-Healing v67 ==="
...
run_mesh_step() {
    ...
    if grep -q 'FOAM FATAL\|Error\|failed' log.blockMesh 2>/dev/null; then
    ...
    if grep -q 'FOAM FATAL\|Error' log.surfaceFeatureExtract 2>/dev/null; then
...
run_sim_step() {
    ...
    if grep -qE 'FOAM FATAL|Segmentation|Error' log.$solver 2>/dev/null; then
        ...
        if diagnose_and_heal "log.$solver" "."; then
            ...
            cp -r 0.orig 0 2>/dev/null || true
            $solver 2>&1 | tee log.${solver}.retry
            if [[ -d 1000 ]]; then
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v67 diagnose_and_heal() for all seen errors, case-check guards,, log dump on failure, self-heal,, no blocking loops, stub fix, OSM terrain pipeline, GE export removed, OSM2World fixes'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
