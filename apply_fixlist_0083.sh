...
# ----------------------------------------------------------------------
# apply_fixlist_0083.sh – Complete self‑healing CFD‑to‑Godot pipeline v83
# v83 : fix generate_terrain_scene check so stale .stl references are overwritten
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self‑Healing v83 ==="
...
# ----------------------------------------------------------------
# Generate terrain.tscn – references the .obj file
# ----------------------------------------------------------------
generate_terrain_scene() {
    local obj_filename="${LOCATION_NAME}.obj"
    local scene_file="godot_project/scenes/terrain.tscn"

    # New check: scene must exist, have MeshInstance3D, AND reference the .obj file
...
# v83: Convert STL to OBJ, copy to Godot, generate scenes (check now detects stale .stl)
convert_stl_to_obj_and_copy
generate_terrain_scene
fix_main_scene
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v83 correct terrain scene check, OBJ reference ensures Godot loads mesh'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

launch_godot_step
llm_audit_info
echo ""
echo "✅ All fixes applied! (v83)"