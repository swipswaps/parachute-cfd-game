...
# ----------------------------------------------------------------------
# apply_fixlist_0002.sh – implements prf.fixlist.v6 on Fedora 43
...
if ! sudo -n true 2>/dev/null; then
    ...
    # We'll still attempt; each sudo command will prompt.
fi
...
run_step "ENV-001 (source_openfoam_env)" \
    "grep -qxF 'source /opt/openfoam10/etc/bashrc' ~/.bashrc" \
    "grep -qxF 'source /opt/openfoam10/etc/bashrc' ~/.bashrc || echo 'source /opt/openfoam10/etc/bashrc' >> ~/.bashrc" \
    "grep -qxF 'source /opt/openfoam10/etc/bashrc' ~/.bashrc"

# ----------------------------------------------------------------------
# PKG-001 – install OpenFOAM
run_step "PKG-001 (install_openfoam)" \
    "bash -c 'source /opt/openfoam10/etc/bashrc 2>/dev/null; command -v simpleFoam'" \
    "sudo dnf install -y openfoam10" \
    "bash -c 'source /opt/openfoam10/etc/bashrc 2>/dev/null; simpleFoam -help 2>&1 | grep -qi openfoam'"
