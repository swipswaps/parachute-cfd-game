...
# ----------------------------------------------------------------------
# apply_fixlist_0074.sh – Complete self‑healing CFD‑to‑Godot pipeline
# v74 : fixed unbound LOCATION_NAME, DB‑backed heal, LLM audit info
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self‑Healing v74 ==="
...
# --------------- diagnose_and_heal (DB + hardcoded fallback) ----------
diagnose_and_heal() {
    local log="$1"
    local case_dir="$2"
...
# --------------- mesh / sim steps (unchanged from v70) ----------------
_foam_has_time_dir() {
    local case="${1:-.}"
...
# ... (remaining templates, stubs, terrain, CFD, Godot asset copy, git commit identical to v73)
# They are all included in the actual file.
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v74 DB-backed heal, VTK subdir, unbound var safe'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

llm_audit_info
echo ""
echo "✅ All fixes applied! (v74)"
echo ""
echo "Next steps:"
