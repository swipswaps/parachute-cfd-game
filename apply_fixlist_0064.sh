...
# ----------------------------------------------------------------------
# apply_fixlist_0064.sh – Complete self-healing CFD-to-Godot pipeline
#
...
REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self-Healing v64 ==="
...
run_mesh_step() {
    ...
    snappyHexMesh -overwrite 2>&1 | tee log.snappyHexMesh | \
        grep --line-buffered -E 'cells|Mesh|Layer|iteration|FOAM FATAL|Error|Checking|Snapping' || true
    ...
    if grep -qE 'did not find.*cell|No cells.*selected|0 cells' log.snappyHexMesh 2>/dev/null; then
        ...
        snappyHexMesh -overwrite 2>&1 | tee log.snappyHexMesh | \
            grep --line-buffered -E 'cells|Mesh|FOAM FATAL|Error' || true
        retried=1
    fi
...
run_sim_step() {
    ...
    $solver 2>&1 | tee log.$solver | \
        grep --line-buffered -E '^Time =|Ux|Uy|Uz| p | k | epsilon|FOAM FATAL|converged|End' || true
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v64 live mesh+sim output, log dump on failure, self-heal,, no blocking loops, stub fix, OSM terrain pipeline, GE export removed, OSM2World fixes'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
