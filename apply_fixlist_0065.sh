...
# ----------------------------------------------------------------------
# apply_fixlist_0065.sh – Complete self-healing CFD-to-Godot pipeline
#
...
REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self-Healing v65 ==="
...
run_step "Template transportProperties (openfoam-2512 compat)" \
    "test -s cases/template/constant/transportProperties && grep -q 'nu' cases/template/constant/transportProperties" \
...
run_step "transportProperties in case (openfoam-2512)" \
    "test -s '${CASE_DIR}/constant/transportProperties'" \
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v65 transportProperties, no grep filter,, log dump on failure, self-heal,, no blocking loops, stub fix, OSM terrain pipeline, GE export removed, OSM2World fixes'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
