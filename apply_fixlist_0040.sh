...
# ----------------------------------------------------------------------
# apply_fixlist_0040.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Kills Google Earth Pro via flatpak and removes stale lock file.
# - No popups, no "already running" errors.
...
# ----------------------------------------------------------------------
# 8. User input (with proper Flatpak kill and lock file removal)
# ----------------------------------------------------------------------
...
if [[ -z "${LOCATION_NAME:-}" ]]; then
    ...
    # Launch Google Earth Pro with clean Flatpak kill and lock removal
    if check_google_earth; then
        ...
        if [[ "$launch_ge" =~ ^[Yy]$ ]]; then
            # Kill any existing Flatpak instance cleanly
            if flatpak ps | grep -q com.google.EarthPro; then
                echo "Killing existing Google Earth Pro via flatpak..."
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (flatpak kill + lock removal)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
