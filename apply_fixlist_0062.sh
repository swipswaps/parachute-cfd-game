...
# ----------------------------------------------------------------------
# apply_fixlist_0062.sh – Complete self-healing CFD-to-Godot pipeline
#
# v62 changes — all derived from confirmed evidence, not guesses:
#
...
REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self-Healing v62 ==="
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v62 all inputs have defaults, no blocking loops, stub fix, OSM terrain pipeline, GE export removed, OSM2World fixes'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
