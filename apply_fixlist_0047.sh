...
# ----------------------------------------------------------------------
# apply_fixlist_0049.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Uses RPM version of Google Earth Pro (no popup bug).
# - Launches GE only once; caches DAE path for subsequent runs.
...
# Flatpak setup (still needed for Godot)
...
# ----------------------------------------------------------------------
# 2. Install Google Earth Pro (RPM version – works, no popup)
# ----------------------------------------------------------------------
...
# Download known working RPM version
...
# Install with --nodeps (bypasses library checks)
run_step "Install Google Earth Pro RPM" \
    "command -v google-earth-pro" \
    "sudo rpm -ivh --nodeps --force /tmp/google-earth-pro-stable.rpm && rm -f /tmp/google-earth-pro-stable.rpm" \
    "command -v google-earth-pro"

# xdotool for raising window (optional but helpful)
run_step "xdotool" \
    "command -v xdotool" \
...
# ----------------------------------------------------------------------
# 5. Stub scripts and documentation (same as v46)
# ----------------------------------------------------------------------
...
if [[ -z "${LOCATION_NAME:-}" ]]; then
    ...
    # Launch Google Earth Pro **only if DAE file does not exist yet**
    if [[ ! -f "$DAE_PATH" ]]; then
        read -r -p "Open Google Earth Pro to export COLLADA file? (y/n, default y): " open_ge
        open_ge=${open_ge:-y}
        if [[ "$open_ge" =~ ^[Yy]$ ]]; then
            echo "Launching Google Earth Pro (RPM version – no popup)..."
            google-earth-pro &
            sleep 3
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (working RPM Google Earth)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
