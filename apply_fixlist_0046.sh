...
# ----------------------------------------------------------------------
# apply_fixlist_0046.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Uses Flatpak Google Earth Pro with force‑kill and lock removal.
# - No popups – always kills stale instance and launches fresh.
...
# ----------------------------------------------------------------------
# 3. OpenFOAM template files (full set – same as v45)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 4. Stub scripts and documentation (same as v45)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 8. User input – force-kill Flatpak Google Earth Pro and launch fresh
# ----------------------------------------------------------------------
...
if [[ -z "${LOCATION_NAME:-}" ]]; then
    ...
    # Force-kill any existing Google Earth Pro instance and remove lock file
    ...
    if [[ "$open_ge" =~ ^[Yy]$ ]]; then
        ...
        rm -f ~/.var/app/com.google.EarthPro/cache/GoogleEarth/instance.lock 2>/dev/null
        sleep 1
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (Flatpak GE with force-kill)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
