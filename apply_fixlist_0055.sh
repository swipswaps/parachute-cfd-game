...
# ----------------------------------------------------------------------
# apply_fixlist_0055.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Installs Google Earth Pro by manually extracting RPM (bypasses digest error).
# - Fixed extraction verification and copy paths.
...
# Install rpm2cpio (part of rpm-build)
run_step "Install rpm2cpio" \
    "command -v rpm2cpio" \
    "sudo dnf install -y rpm-build" \
    "command -v rpm2cpio"

# Extract RPM manually – fixed check path
run_step "Extract Google Earth Pro RPM" \
    "test -f /tmp/ge_extract/opt/google/earth/pro/google-earth-pro" \
    "mkdir -p /tmp/ge_extract && cd /tmp/ge_extract && rpm2cpio /tmp/google-earth-pro-stable.rpm | cpio -idmv" \
    "test -f /tmp/ge_extract/opt/google/earth/pro/google-earth-pro"

# Copy extracted files to /usr and clean up – fixed copy command
run_step "Install extracted Google Earth Pro" \
    "command -v google-earth-pro" \
    "sudo cp -r /tmp/ge_extract/opt / && sudo cp -r /tmp/ge_extract/usr/* /usr/ && sudo rm -rf /tmp/ge_extract && rm -f /tmp/google-earth-pro-stable.rpm" \
    "command -v google-earth-pro"

# Symlink
sudo ln -sf /opt/google/earth/pro/google-earth-pro /usr/local/bin/google-earth-pro

# xdotool (still useful)
run_step "xdotool" \
    "command -v xdotool" \
...
# ----------------------------------------------------------------------
# 4. OpenFOAM template files (full set – same as v54)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 5. Stub scripts and documentation (same as v54)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 9. User input – launch Google Earth Pro only if DAE missing (RPM version, no popups)
# ----------------------------------------------------------------------
...
if [[ -z "${LOCATION_NAME:-}" ]]; then
    ...
    # Launch Google Earth Pro only if DAE file is missing (RPM version – no popup)
    if [[ ! -f "$DAE_PATH" ]]; then
        read -r -p "Open Google Earth Pro to export COLLADA file? (y/n, default y): " open_ge
        open_ge=${open_ge:-y}
        if [[ "$open_ge" =~ ^[Yy]$ ]]; then
            echo "Launching Google Earth Pro (RPM version – no popups)..."
            google-earth-pro &
            sleep 3
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (extraction paths fixed)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
