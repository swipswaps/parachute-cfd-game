...
# ----------------------------------------------------------------------
# apply_fixlist_0082.sh – Complete self‑healing CFD‑to‑Godot pipeline v82
# v82 : copy OBJ to Godot assets after STL conversion
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self‑Healing v82 ==="
...
# v82: Convert STL to OBJ, copy to Godot assets, generate scenes
convert_stl_to_obj_and_copy
generate_terrain_scene
fix_main_scene
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v82 OBJ copy to Godot assets, full pipeline restored'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

launch_godot_step
llm_audit_info
echo ""
echo "✅ All fixes applied! (v82)"