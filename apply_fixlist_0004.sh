...
# ----------------------------------------------------------------------
# apply_fixlist_0004.sh – implements prf.fixlist.v6 without hanging PKG-001
# OpenFOAM is assumed already installed (verified manually).
# All other steps (templates, stubs, docs, test, commit) run normally.
# ----------------------------------------------------------------------
...
# Use a flexible pattern that matches the installed version
run_step "ENV-001 (source_openfoam_env)" \
    "grep -q 'source /usr/lib/openfoam/openfoam' ~/.bashrc" \
    "if ! grep -q 'source /usr/lib/openfoam/openfoam' ~/.bashrc; then echo 'source /usr/lib/openfoam/openfoam*/etc/bashrc' >> ~/.bashrc; fi" \
    "grep -q 'source /usr/lib/openfoam/openfoam' ~/.bashrc"
