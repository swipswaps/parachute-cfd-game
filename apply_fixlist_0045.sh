...
# ----------------------------------------------------------------------
# apply_fixlist_0045.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Installs Google Earth Pro RPM manually (bypasses DNF digest errors).
...
# Add RPM repository (optional – for future updates, but we'll install manually)
...
# Install Google Earth Pro manually (bypass DNF digest errors)
run_step "Google Earth Pro manual RPM install" \
    "command -v google-earth-pro" \
    "sudo dnf clean all && wget -O /tmp/google-earth-pro-stable.rpm https://dl.google.com/linux/earth/rpm/stable/x86_64/google-earth-pro-stable-current.x86_64.rpm && sudo rpm -ivh --nodeps --nosignature /tmp/google-earth-pro-stable.rpm && rm -f /tmp/google-earth-pro-stable.rpm" \
    "command -v google-earth-pro"
...
# ----------------------------------------------------------------------
# 3. OpenFOAM template files (full set – same as v44)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 4. Stub scripts and documentation (same as v44)
# ----------------------------------------------------------------------
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (manual RPM install)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
