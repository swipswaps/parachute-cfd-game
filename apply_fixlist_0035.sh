...
# ----------------------------------------------------------------------
# apply_fixlist_0035.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Installs Google Earth Pro automatically if missing.
# - Improved guidance for manual COLLADA export with retry loop.
# - Includes full OpenFOAM templates and CFD pipeline (no omissions).
...
# ----------------------------------------------------------------------
# 8. User input (with improved Google Earth guidance and retry loop)
# ----------------------------------------------------------------------
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (improved GE export)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
