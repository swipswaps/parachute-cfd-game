...
# ----------------------------------------------------------------------
# apply_fixlist_0008.sh – Fully compliant with prf.fixlist.v6
# - Detects OpenFOAM bashrc path but does NOT source it automatically.
# - Adds an alias 'ofsrc' to ~/.bashrc for manual activation.
# - Installs Flatpak and adds Flathub remote before installing Godot.
# - Fixes syntax error in DOC-001 (populate_empty_docs) using proper Python.
...
# ----------------------------------------------------------------------
# DOC-001 – populate empty docs (fixed syntax: no for loop, explicit per-file)
...
run_step "GIT-001 (commit_all_fixes)" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply prf.fixlist.v6 — deterministic OpenFOAM templates, GDAL deps, alias, stubs, docs')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
