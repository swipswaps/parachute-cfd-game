...
# ----------------------------------------------------------------------
# apply_fixlist_0057.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - v57: Fix extraction step by adding chmod +x and using -f check.
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self‑Healing v57 ==="
...
# ----------------------------------------------------------------------
# 2. Install Google Earth Pro via manual extraction (bypass digest errors)
# ----------------------------------------------------------------------
...
# Remove any existing RPM package
...
# --- v57: Force clean extraction, chmod +x, check file exists ---
run_step "Extract Google Earth Pro RPM" \
    "test -f /tmp/ge_extract/opt/google/earth/pro/google-earth-pro" \
    "sudo rm -rf /tmp/ge_extract && mkdir -p /tmp/ge_extract && cd /tmp/ge_extract && rpm2cpio /tmp/google-earth-pro-stable.rpm | cpio -idm && chmod +x /tmp/ge_extract/opt/google/earth/pro/google-earth-pro" \
    "test -f /tmp/ge_extract/opt/google/earth/pro/google-earth-pro"

# Copy extracted files and set permissions
run_step "Install extracted Google Earth Pro" \
    "command -v google-earth-pro" \
    "sudo cp -r /tmp/ge_extract/opt / && sudo cp -r /tmp/ge_extract/usr/* /usr/ && sudo rm -rf /tmp/ge_extract && rm -f /tmp/google-earth-pro-stable.rpm && sudo chmod +x /opt/google/earth/pro/google-earth-pro" \
    "test -x /opt/google/earth/pro/google-earth-pro"

# Ensure symlink
run_step "Symlink google-earth-pro" \
    "test -x /usr/local/bin/google-earth-pro" \
    "sudo ln -sf /opt/google/earth/pro/google-earth-pro /usr/local/bin/google-earth-pro" \
    "command -v google-earth-pro"
...
# ----------------------------------------------------------------------
# 9. User input – launch Google Earth Pro
# ----------------------------------------------------------------------
...
if [[ -z "${LOCATION_NAME:-}" ]]; then
    ...
    echo ""
    echo "  Option B – Manual search (if you don't have a placemark):"
    ...
    echo ""
    echo "  Option C – Using RenderDoc (for terrain exactly as seen):"
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: v57 correct extraction verify and permissions')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

echo ""
echo "✅ All fixes applied successfully! (v57)"
echo ""
