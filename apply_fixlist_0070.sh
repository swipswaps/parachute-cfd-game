...
# ----------------------------------------------------------------------
# apply_fixlist_0070.sh – Complete self-healing CFD-to-Godot pipeline
#
# v64 changes — all derived from confirmed evidence, not guesses:
#
...
REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self-Healing v70 ==="
...
# ─────────────────────────────────────────────────────────────────────
# _foam_has_time_dir: returns 0 if ANY numeric time dir > 0 exists.
# Handles early convergence — solver may stop at T=121, not T=1000.
# Evidence: simpleFoam converged at T=121, wrote dir "121" not "1000".
# ─────────────────────────────────────────────────────────────────────
_foam_has_time_dir() {
    local case="${1:-.}"
...
# ─────────────────────────────────────────────────────────────────────
# HEAL DATABASE: SQLite-backed persistent error→fix registry.
# .cfd_healdb stores known patterns, fix commands, success/fail counts.
# Scripts written to /tmp at startup. DB consulted before hardcoded rules.
# New unknown errors recorded for future pattern development.
# ─────────────────────────────────────────────────────────────────────
HEAL_DB=".cfd_healdb"

# Write heal DB helper scripts to /tmp (idempotent — overwrite is fine)
...
db_diagnose_and_heal() {
    ...
    case "$result" in
        NO_DB|NO_LOG|USAGE)
            echo "  ℹ️  DB unavailable — falling through to hardcoded rules"
            return 1 ;;
        UNKNOWN)
            echo "  ⚠️  Unknown error — recorded in DB for future pattern development"
            echo "      Review: python3 /tmp/cfd_db_diagnose.py $db --show-unknown"
            return 1 ;;
        MATCHED:*)
            ...
            if eval "$fix_cmd"; then
                ...
                echo "  ✅ DB heal applied and recorded (success_count++)"
                return 0
            else
                python3 /tmp/cfd_db_diagnose.py "$db" --record-outcome "$pid" "failed" "$case_dir"
                echo "  ❌ DB heal fix failed (fail_count++)"
                return 1
            fi ;;
    esac
}
...
# ─────────────────────────────────────────────────────────────────────
# diagnose_and_heal: reads a foam log, matches known error patterns,
# applies self-healing fix in place, returns 0 if healed (caller retries)
# or 1 if unknown error (caller exits).
# Principle: every known error seen in this session is handled here.
# ─────────────────────────────────────────────────────────────────────
diagnose_and_heal() {
    local log="$1"
    local case_dir="$2"
    ...
    # ── TRANSPORT-MISSING ─────────────────────────────────────────────
    # "cannot find file .../constant/transportProperties"
...
'FoamFile\n{ version 2.0; format ascii; class dictionary; object transportProperties; }\ntransportModel Newtonian;\nnu nu [0 2 -1 0 0 0 0] 1.5e-05;\n')"
        echo "  ✅ transportProperties written"
        return 0
    fi

    # ── TRANSPORT-WRONG-KEY ───────────────────────────────────────────
    # "Entry 'transportModel' not found in dictionary .../transportProperties"
...
open('$case_dir/constant/transportProperties','w').write(
'FoamFile\n{ version 2.0; format ascii; class dictionary; object transportProperties; }\ntransportModel Newtonian;\nnu nu [0 2 -1 0 0 0 0] 1.5e-05;\n')"
        echo "  ✅ transportProperties overwritten (transportModel keyword)"
        return 0
    fi
...
'FoamFile\n{ version 2.0; format ascii; class dictionary; object physicalProperties; }\nviscosityModel Newtonian;\nnu 1.5e-05;\n')"
        echo "  ✅ physicalProperties written"
        return 0
    fi
...
'FoamFile\n{ version 2.0; format ascii; class dictionary; object turbulenceProperties; }\nsimulationType RAS;\nRAS { RASModel kEpsilon; turbulence on; printCoeffs on; }\n')"
        echo "  ✅ turbulenceProperties written"
        return 0
    fi
    ...
    if grep -q "Cannot open mesh description\|polyMesh/boundary" "$log" 2>/dev/null; then
        ...
        if [[ -d "$case_dir/constant/polyMesh" ]]; then
            ...
            echo "  ❌ mesh regeneration failed — check log.Allrun.mesh.reheal"
            return 1
        fi
    fi
    ...
    if grep -qE "Cannot find patchField entry|patch.*not found in field" "$log" 2>/dev/null; then
        local missing_patch
        missing_patch=$(grep -oE "patch '([^']+)'" "$log" | head -1 | tr -d "'patch ")
        local missing_field
        missing_field=$(grep -oE "field (p|U|k|epsilon)" "$log" | head -1 | awk '{print $2}')
    ...
    # ── NUT-MISSING ───────────────────────────────────────────────────
    # "cannot find file .../0/nut" — k-epsilon turbulent viscosity field
...
')"
    ...
    # ── OMEGA-MISSING ─────────────────────────────────────────────────
    # "cannot find file .../0/omega" — k-omega SST specific field
...
')"
        echo "  ✅ 0/omega written"
        return 0
    fi

    # ── GENERIC-MISSING-FIELD ─────────────────────────────────────────
    # "cannot find file .../0/<field>" — catch any other missing initial field
    local missing_field
    missing_field=$(grep -oE "cannot find file.*/0/[a-zA-Z]+" "$log" 2>/dev/null | grep -oE "/0/[a-zA-Z]+" | head -1 | tr -d '/0/')
...
')"
        echo "  ✅ 0/$missing_field written (zero placeholder — may need manual adjustment)"
        return 0
    fi
...
# ─────────────────────────────────────────────────────────────────────
# run_mesh_step: runs blockMesh + surfaceFeatureExtract + snappyHexMesh
# with live output, log dump on failure, and self-healing retry.
# Principle: if it can be typed, it MUST be scripted. Never hide output.
# ─────────────────────────────────────────────────────────────────────
run_mesh_step() {
    local case_dir="$1"
    ...
    if grep -q 'FOAM FATAL\|FOAM exiting\|error:' log.surfaceFeatureExtract 2>/dev/null; then
        echo "  ❌ surfaceFeatureExtract failed"
        _foam_log_dump log.surfaceFeatureExtract 30
        cd - >/dev/null; exit 1
    fi
    echo "  ✅ surfaceFeatureExtract OK"

    # ── snappyHexMesh with live cell-count progress ───────────────────
    echo "  🔧 snappyHexMesh (2-10 min — live progress below)..."
    # No grep filter — show all snappyHexMesh output verbatim immediately
    ...
    # Pattern 1: locationInMesh inside solid geometry
    if grep -qE 'did not find.*cell|No cells.*selected|0 cells' log.snappyHexMesh 2>/dev/null; then
        echo "  DIAGNOSIS: locationInMesh (150 150 100) is inside solid geometry."
        echo "  FIX: Moving locationInMesh to z=180m (above buildings)..."
        sed -i 's/locationInMesh (150 150 100)/locationInMesh (150 150 180)/' system/snappyHexMeshDict
        echo "  Re-running snappyHexMesh..."
        snappyHexMesh -overwrite 2>&1 | tee log.snappyHexMesh
        retried=1
    fi

    # Pattern 2: OOM or cell limit
    if grep -qE 'FOAM FATAL|bad_alloc|Killed|cell limit' log.snappyHexMesh 2>/dev/null && [[ $retried -eq 0 ]]; then
        echo "  DIAGNOSIS: Out of memory or cell limit exceeded."
        echo "  FIX: Halving maxLocalCells and maxGlobalCells..."
        sed -i 's/maxLocalCells 1000000/maxLocalCells 500000/' system/snappyHexMeshDict
        sed -i 's/maxGlobalCells 2000000/maxGlobalCells 1000000/' system/snappyHexMeshDict
        echo "  Re-running snappyHexMesh with reduced cell limits..."
        snappyHexMesh -overwrite 2>&1 | tee log.snappyHexMesh
        retried=1
    fi

    if [[ -d constant/polyMesh ]]; then
        echo "  ✅ snappyHexMesh OK after retry"
        cd - >/dev/null; return 0
    fi

    if [[ $retried -eq 1 ]]; then
        echo "  ❌ snappyHexMesh still failed after retry"
        _foam_log_dump log.snappyHexMesh 40
    fi

    echo "  Full log: $case_dir/log.snappyHexMesh"
    cd - >/dev/null; exit 1
}

# ─────────────────────────────────────────────────────────────────────
# run_sim_step: runs solver with collapsed numbered residual display.
# Identical per-iteration lines collapsed to one updating summary.
# Every 10 steps: persistent numbered line. Full log always written.
# Real-time detection: CONVERGED, DIVERGED, STALLED (50 non-improving).
# ─────────────────────────────────────────────────────────────────────
run_sim_step() {
    local case_dir="$1"
    ...
    # stdbuf -oL: line-buffered output — no 64KB stall on fast failure
    # || true: set -e guard — allows diagnose_and_heal to run after failure
    { $solver 2>&1 | stdbuf -oL tee log.$solver; } || true

    # ── Collapsed display: one line per time step ─────────────────────
    # Reads completed log, collapses 8 lines/iter into numbered summary.
    # Numbered (#N) every 10 iters; others overwrite previous line (\r).
    awk '
    ...
    # ── Fatal error: dump log + heal + retry ──────────────────────────
    if grep -qE 'FOAM FATAL|Segmentation fault' log.$solver 2>/dev/null; then
        ...
        if diagnose_and_heal "log.$solver" "."; then
            echo "  🔄 Retrying $solver after heal..."
            cp -r 0.orig 0 2>/dev/null || true
    ...
    # ── Stall: non-fatal, continues with available results ────────────
    if awk '
    ...
    ' log.$solver 2>/dev/null; then
        echo "  ⚠️  STALL: residuals stopped improving (50+ iters). Last 20 lines:"
        tail -20 log.$solver
        echo "  To fix: increase endTime or reduce relaxation factors in system/fvSolution."
        echo "  Continuing with available results..."
    fi

    if ! _foam_has_time_dir .; then
        echo "  ❌ no time directory 1000 produced"
        tail -40 log.$solver
        cd - >/dev/null; exit 1
    fi
...
# Initialize heal database and write helper scripts
init_heal_db
...
# TRANSPORT-001: openfoam-2512 (ESI) requires transportProperties.
# physicalProperties is OpenFOAM.org v10+ only. Write BOTH for compatibility.
# Evidence: FOAM FATAL ERROR: cannot find file constant/transportProperties
...
# NUT-MISSING-001: ensure 0/nut exists in case — k-epsilon requires it.
# setup_openfoam_case.sh copies 0.orig/ but only if template has it.
# Write directly to case 0/ to handle existing cases from prior runs.
...
# TRANSPORT-001: ensure transportProperties exists in the case (not just template)
# openfoam-2512 requires this file — it is NOT created by setup_openfoam_case.sh
# when the case already exists from a prior run.
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v70 heal DB, time-dir early-convergence fix, log dump on failure, self-heal,, no blocking loops, stub fix, OSM terrain pipeline, GE export removed, OSM2World fixes'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

echo ""
echo "✅ All fixes applied! (v70)"
echo ""
echo "Next steps:"
