...
# ----------------------------------------------------------------------
# apply_fixlist_0003.sh – implements prf.fixlist.v6 with correct OpenFOAM package name for Fedora
# Idempotent, deterministic, self‑healing.
# Run from the repository root where README.md and scripts/ exist.
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Applying fixes in: $REPO_ROOT ==="

# Guard: must be run from the correct repo root
if [[ ! -f README.md ]] || [[ ! -d scripts ]]; then
    echo "ERROR: This script must be run from the root of the parachute-cfd-game repository."
    exit 1
fi

# Check sudo availability (will be needed for package installs)
if ! sudo -n true 2>/dev/null; then
    echo "This script requires sudo privileges to install packages."
    echo "Please run with a user that has sudo access, and ensure you have a passwordless sudo or be ready to enter password."
fi

# Helper: run a step with check, apply, verify
run_step() {
    local step_name="$1"
    local check_cmd="$2"
    local apply_cmd="$3"
...
# ----------------------------------------------------------------------
# ENV-001 – source OpenFOAM env in .bashrc (idempotent exact line)
run_step "ENV-001 (source_openfoam_env)" \
    "grep -qxF 'source /usr/lib/openfoam/openfoam*/etc/bashrc' ~/.bashrc || grep -qxF 'source /opt/openfoam*/etc/bashrc' ~/.bashrc" \
    "if ! grep -qxF 'source /usr/lib/openfoam/openfoam*/etc/bashrc' ~/.bashrc; then echo 'source /usr/lib/openfoam/openfoam*/etc/bashrc' >> ~/.bashrc; fi" \
    "grep -q 'openfoam' ~/.bashrc"

# ----------------------------------------------------------------------
# PKG-001 – install OpenFOAM (correct package name: 'openfoam')
run_step "PKG-001 (install_openfoam)" \
    "bash -c 'source /usr/lib/openfoam/openfoam*/etc/bashrc 2>/dev/null || source /opt/openfoam*/etc/bashrc 2>/dev/null; command -v simpleFoam'" \
    "sudo dnf install -y openfoam" \
    "bash -c 'source /usr/lib/openfoam/openfoam*/etc/bashrc 2>/dev/null || source /opt/openfoam*/etc/bashrc 2>/dev/null; simpleFoam -help 2>&1 | grep -qi openfoam'"
...
# ----------------------------------------------------------------------
# PKG-003 – install Godot Flatpak
run_step "PKG-003 (install_godot)" \
    "command -v godot" \
...
# ----------------------------------------------------------------------
# PY-001 – Python packages
run_step "PY-001 (install_python_packages)" \
    "python3 -c 'import trimesh, vtk, pykml, lxml, osgeo, scipy, matplotlib' 2>/dev/null" \
    "python3 -m pip install --user --break-system-packages numpy vtk trimesh pykml lxml GDAL scipy matplotlib" \
    "python3 -c 'import trimesh, vtk, pykml, lxml, osgeo, scipy, matplotlib; print(\"OK\")'"

# ----------------------------------------------------------------------
# EXEC-001 – set execute bits on scripts (idempotent, handles empty dir)
...
# ----------------------------------------------------------------------
# DOC-001 – populate empty documentation files
...
# ----------------------------------------------------------------------
# READ-002 – clean docs/file_structure.md if it exists
...
# ----------------------------------------------------------------------
# TEST-001 – verify collada_to_stl with a cube (always runs)
run_step "TEST-001 (verify_collada_to_stl_with_cube)" \
    "false" \
    "python3 -c 'import trimesh; cube = trimesh.creation.box(extents=[100,100,10]); cube.export(\"test_cube.dae\")' && python3 scripts/collada_to_stl.py --input test_cube.dae --output test_cube.stl --simplify 0.5" \
...
run_step "GIT-001 (commit_all_fixes)" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply prf.fixlist.v6 — deterministic OpenFOAM templates, GDAL deps, env, stubs, docs')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

echo "✅ All fixes applied successfully! Your repository is now self‑healed and ready for CFD simulation."