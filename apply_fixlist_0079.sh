...
# ----------------------------------------------------------------------
# apply_fixlist_0079.sh – Complete self‑healing CFD‑to‑Godot pipeline
# v79 : fix still‑broken main.tscn by generating a valid minimal scene
#       with terrain instance, light, camera; heal DB extended.
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self‑Healing v79 ==="
...
# ----------------------------------------------------------------
# Generate valid main.tscn with terrain instance, light, camera
# ----------------------------------------------------------------
fix_main_scene() {
...
# v79: Generate terrain.tscn and fix main.tscn
generate_terrain_scene
fix_main_scene
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v79 generate valid main.tscn with terrain instance, light, camera'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

launch_godot_step
llm_audit_info
echo ""
echo "✅ All fixes applied! (v79)"