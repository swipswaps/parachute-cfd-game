...
# ----------------------------------------------------------------------
# apply_fixlist_0014.sh – Idempotent, self‑healing CFD‑to‑Godot automation
# Principle: "If it can be typed, it MUST be scripted!"
# Style: deterministic checks, idempotent steps, resumable, auto‑env.
# ----------------------------------------------------------------------
...
if ! command -v simpleFoam &>/dev/null; then
    ...
    # Source directly – this is safe and will affect the rest of the script
...
# ----------------------------------------------------------------------
# 1. User input – cached in .last_run to avoid re‑asking
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 3. Convert COLLADA → STL (idempotent: check output STL exists and is newer?)
#    For simplicity, we check existence; you could add `-nt` check.
# ----------------------------------------------------------------------
...
#    We assume simulation writes to e.g., 1000/ (endTime=1000 in controlDict)
# ----------------------------------------------------------------------
run_step "Run isothermal simulation" \
    "test -d '$CASE_DIR/1000' || test -f '$CASE_DIR/postProcessing/residuals/0/residuals.dat'" \
...
#    We copy the latest JSON and DAE, but keep previous versions in .bak
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 10. Final instructions (no automated Godot launch, but offer)
# ----------------------------------------------------------------------
echo ""
