...
# ----------------------------------------------------------------------
# apply_fixlist_0054.sh – Complete self‑healing CFD‑to‑Godot pipeline
...
# Extract RPM manually
run_step "Extract Google Earth Pro RPM" \
    "test -f /tmp/ge_extract/usr/bin/google-earth-pro" \
    "mkdir -p /tmp/ge_extract && cd /tmp/ge_extract && rpm2cpio /tmp/google-earth-pro-stable.rpm | cpio -idmv" \
    "test -f /tmp/ge_extract/usr/bin/google-earth-pro"

# Copy extracted files to /usr and clean up
run_step "Install extracted Google Earth Pro" \
    "command -v google-earth-pro" \
    "sudo cp -r /tmp/ge_extract/usr/* /usr/ && sudo rm -rf /tmp/ge_extract && rm -f /tmp/google-earth-pro-stable.rpm" \
    "command -v google-earth-pro"
...
# ----------------------------------------------------------------------
# 4. OpenFOAM template files (full set – same as v53)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 5. Stub scripts and documentation (same as v53)
# ----------------------------------------------------------------------
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (manual RPM extraction)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
