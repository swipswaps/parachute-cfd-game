...
# ----------------------------------------------------------------------
# apply_fixlist_0036.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Uses xdotool to send search to existing Google Earth Pro window.
# - Installs xdotool automatically.
# - Full OpenFOAM templates and CFD pipeline included.
# - Idempotent step design (check → apply → verify).
# ----------------------------------------------------------------------
...
# Check if Google Earth Pro is installed (used later)
check_google_earth() {
...
# ----------------------------------------------------------------------
# 8. User input (with xdotool integration for existing Google Earth window)
# ----------------------------------------------------------------------
...
if [[ -z "${LOCATION_NAME:-}" ]]; then
    ...
    # Launch or focus Google Earth Pro with search
    if check_google_earth; then
        ...
        if [[ "$launch_ge" =~ ^[Yy]$ ]]; then
            if pgrep -f "google-earth-pro" >/dev/null; then
                echo "Google Earth Pro already running – bringing to front and sending search."
                WINDOW=$(xdotool search --name "Google Earth" | head -1)
                if [[ -n "$WINDOW" ]]; then
                    ...
                    sleep 0.5
                    xdotool key Return
                    echo "Search sent to Google Earth Pro."
                else
            ...
            echo "Google Earth Pro is ready. Please zoom to ~300-500m and ensure 3D Buildings layer is enabled."
            read -r -p "Press Enter after you have located the area and are ready to export..."
            echo ""
            echo "After exporting the DAE file, you will be prompted to enter its path."
            echo "If the export fails, you can type ‘retry’ to launch Google Earth again."
        fi
    else
    ...
    while true; do
        if [[ -z "$DAE_PATH" || ! -f "$DAE_PATH" ]]; then
            read -r -p "Path to COLLADA (.dae) file (or type ‘retry’ to reopen Google Earth): " DAE_PATH
            if [[ "$DAE_PATH" == "retry" ]]; then
                google-earth-pro --search "$SEARCH_QUERY" &
                echo "Google Earth Pro reopened. After exporting, enter the file path again."
                DAE_PATH=""
                continue
            fi
            if [[ -f "$DAE_PATH" ]]; then
                ...
                echo "File not found. Please provide a valid path or type ‘retry’."
            fi
        else
            break
        fi
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (xdotool for GE)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
