...
# ----------------------------------------------------------------------
# apply_fixlist_0091.sh – Complete self‑healing CFD‑to‑Godot pipeline v91
# v91 : adds unknown error logging and fuzzy matching fallback
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Self‑Healing v91 (full) ==="
...
# --------------- heal database (with escaped newlines) ---------------
...
run_mesh_step() {
    ...
    if [ -f "$case_dir/Allrun.mesh" ]; then
        ...
        echo "  ❌ No Allrun.mesh, using default blockMesh+snappyHexMesh"
        (cd "$case_dir" && blockMesh 2>&1 | tee -a "$log_file" && snappyHexMesh -overwrite 2>&1 | tee -a "$log_file") || true
    fi
    diagnose_and_heal "$log_file" "$case_dir" && { echo "  ✅ Mesh generation succeeded"; return 0; } || { echo "  🔄 Re-running mesh..."; (cd "$case_dir" && blockMesh 2>&1 | tee -a "$log_file" && snappyHexMesh -overwrite 2>&1 | tee -a "$log_file") || return 1; }
}
...
main() {
    ...
    launch_godot_step
    perform_llm_audit
    echo "=== All steps completed successfully (v91) ==="
}
