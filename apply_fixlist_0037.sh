...
# ----------------------------------------------------------------------
# apply_fixlist_0037.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Simplified Google Earth Pro launch (no xdotool, no search automation).
# - User navigates manually after the app opens.
...
# ----------------------------------------------------------------------
# 8. User input (simplified Google Earth launch)
# ----------------------------------------------------------------------
...
if [[ -z "${LOCATION_NAME:-}" ]]; then
    ...
    # Launch Google Earth Pro if available (simple, no search automation)
    if check_google_earth; then
        ...
        if [[ "$launch_ge" =~ ^[Yy]$ ]]; then
            google-earth-pro &
            echo "Google Earth Pro launched. Please navigate to your area manually (use search or zoom)."
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (simplified GE launch)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
