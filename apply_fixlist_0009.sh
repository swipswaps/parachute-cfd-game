...
# ----------------------------------------------------------------------
# apply_fixlist_0009.sh – Fully compliant with prf.fixlist.v6
...
# - Adds pycollada to Python packages (required for trimesh .dae export).
...
# ----------------------------------------------------------------------
# PY-001 – Python packages (including trimesh and pycollada)
run_step "PY-001 (install_python_packages)" \
    "python3 -c 'import trimesh, vtk, pykml, lxml, osgeo, scipy, matplotlib, collada' 2>/dev/null" \
    "python3 -m pip install --user --break-system-packages numpy vtk trimesh pykml lxml GDAL scipy matplotlib pycollada" \
    "python3 -c 'import trimesh, vtk, pykml, lxml, osgeo, scipy, matplotlib, collada; print(\"OK\")'"
...
# ----------------------------------------------------------------------
# TEST-001 – verify collada_to_stl with a cube (requires trimesh + pycollada)
...
run_step "GIT-001 (commit_all_fixes)" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply prf.fixlist.v6 — deterministic OpenFOAM templates, GDAL deps, alias, stubs, docs, pycollada')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
