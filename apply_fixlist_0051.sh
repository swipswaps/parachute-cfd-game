...
# ----------------------------------------------------------------------
# apply_fixlist_0051.sh – Complete self‑healing CFD‑to‑Godot pipeline
# - Launches Google Earth Pro (Flatpak) WITHOUT the "another instance" popup
#   by forcibly killing any existing instance, removing lock files,
#   and starting a fresh process.
# - Updates the export instructions to reflect the actual Google Earth Pro UI:
#   * Use right‑click on a placemark in the Places panel → "Save Place As…"
#   * The saved .kmz file contains a COLLADA .dae when 3D models are present.
#   * Fallback: use RenderDoc to capture 3D geometry if needed.
...
# ----------------------------------------------------------------------
# 2. Install Google Earth Pro (Flatpak – the only reliable version)
# ----------------------------------------------------------------------
# Remove any broken RPM version if present
if rpm -q google-earth-pro-stable 2>/dev/null; then
    echo "Removing RPM version of Google Earth Pro..."
    sudo rpm -e google-earth-pro-stable 2>/dev/null || true
fi

# Ensure Flatpak Google Earth Pro is installed
run_step "Google Earth Pro Flatpak" \
    "flatpak list | grep -q com.google.EarthPro" \
    "flatpak install -y flathub com.google.EarthPro" \
    "flatpak list | grep -q com.google.EarthPro"

# xdotool (optional, for window focus)
run_step "xdotool" \
    "command -v xdotool" \
...
# ----------------------------------------------------------------------
# 4. OpenFOAM template files (full set)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 9. User input – launch Google Earth Pro only if DAE missing, with proper cleanup
# ----------------------------------------------------------------------
...
if [[ -z "${LOCATION_NAME:-}" ]]; then
    ...
    echo "  Option A – Using a placemark (easiest):"
    echo "    1. In the left 'Places' panel, right‑click on any folder or placemark."
    echo "    2. Select 'Save Place As...'."
    echo "    3. Choose a filename. The file will be saved as .kmz (a zip archive)."
    echo "    4. Unzip the .kmz file – inside you will find a COLLADA .dae file."
    echo ""
    echo "  Option B – Using RenderDoc (for terrain exactly as seen):"
    echo "    1. Install RenderDoc: sudo dnf install renderdoc"
    echo "    2. Run: qrenderdoc"
    echo "    3. Launch google-earth-pro from RenderDoc, capture a frame."
    echo "    4. Export the mesh as .dae or .obj, then use collada_to_stl.py."
    echo ""
    echo "  Option C – Manual search (if you don't have a placemark):"
    echo "    1. Search for your location (e.g., 'Skydive DeLand')."
    echo "    2. Zoom to ~300-500m width, enable 3D Buildings."
    echo "    3. File → Save → Save Place As... (this saves a .kmz of the current view)."
    echo "    4. Unzip the .kmz to extract the .dae file."
    echo ""

    # Launch Google Earth Pro only if DAE file is missing (with clean instance startup)
    if [[ ! -f "$DAE_PATH" ]]; then
        read -r -p "Open Google Earth Pro to export COLLADA file? (y/n, default y): " open_ge
        open_ge=${open_ge:-y}
        if [[ "$open_ge" =~ ^[Yy]$ ]]; then
            # Force‑clean any existing Google Earth Pro Flatpak instance
            echo "Killing any existing Google Earth Pro process and removing lock..."
            flatpak kill com.google.EarthPro 2>/dev/null || true
            rm -f ~/.var/app/com.google.EarthPro/cache/GoogleEarth/instance.lock 2>/dev/null || true
            sleep 2
            echo "Launching fresh Google Earth Pro..."
            flatpak run com.google.EarthPro &
            sleep 3
            echo "Google Earth Pro launched (fresh instance)."
            echo "Follow the instructions above to export the COLLADA (.dae) file."
...
run_step "Commit all changes" \
    ...
    "git add -A && (git diff --staged --quiet || git commit -m 'fix: apply complete self‑healing pipeline (Flatpak GE with proper force-clean)')" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"
