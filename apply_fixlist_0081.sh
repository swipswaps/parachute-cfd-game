...
# ----------------------------------------------------------------------
# apply_fixlist_0081.sh – Complete self‑healing CFD‑to‑Godot pipeline
# v81 : full restoration of all functions, STL‑to‑OBJ for Godot
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self‑Healing v81 ==="
...
PATTERNS = [
    ...
    ("Parse Error:.*\\.tscn","Godot scene parse error – regenerate minimal main.tscn","bash -c 'mkdir -p godot_project/scenes && cat > godot_project/scenes/main.tscn << \"MAINTSCN\"\n[gd_scene load_steps=2 format=3]\n\n[ext_resource type=\"PackedScene\" path=\"res://scenes/terrain.tscn\" id=\"1_terrain\"]\n\n[node name=\"Main\" type=\"Node3D\"]\n\n[node name=\"WorldEnvironment\" type=\"WorldEnvironment\" parent=\".\"]\n\n[node name=\"DirectionalLight3D\" type=\"DirectionalLight3D\" parent=\".\"]\ntransform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, 0, 10, 5)\n\n[node name=\"TerrainInstance\" type=\"Node3D\" parent=\".\"]\n[node name=\"Terrain\" parent=\"TerrainInstance\" instance=ExtResource(\"1_terrain\")]\nMAINTSCN\n'"),
    ("No loader found for resource.*\\.stl","Godot cannot load STL – convert to OBJ and update scene","bash -c 'STL_PATH=$(grep -oP \"path=\\\"res://[^\\\"]*\\.stl\\\"\" {case}/scenes/terrain.tscn | cut -d\\\" -f2 | sed \"s|res://|godot_project/|g\"); if [ -f \"$STL_PATH\" ]; then python3 -c \"import trimesh; trimesh.load(\\\"$STL_PATH\\\").export(\\\"${STL_PATH%.stl}.obj\\\")\"; sed -i \"s|\\.stl|\.obj|g\" {case}/scenes/terrain.tscn; fi'"),
]
...
# ----------------------------------------------------------------
# Convert STL to OBJ for Godot compatibility
# ----------------------------------------------------------------
convert_stl_to_obj() {
    local stl_file="${STL_PATH}"   # e.g., cfd_mesh/skydive_deland.stl
    local obj_file="${stl_file%.stl}.obj"
    run_step "Convert STL to OBJ (Godot compatible mesh)" \
        "test -f '$obj_file'" \
...
# ----------------------------------------------------------------
# Generate terrain.tscn – now references the .obj file
# ----------------------------------------------------------------
generate_terrain_scene() {
    local obj_filename="${LOCATION_NAME}.obj"   # skydive_deland.obj
...
# ----------------------------------------------------------------
# Generate valid main.tscn (unchanged from v79)
# ----------------------------------------------------------------
fix_main_scene() {
...
# v81: Convert STL to OBJ, then generate scenes
convert_stl_to_obj
generate_terrain_scene
fix_main_scene
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v81 full restoration, STL->OBJ for Godot, updated scenes'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

launch_godot_step
llm_audit_info
echo ""
echo "✅ All fixes applied! (v81)"