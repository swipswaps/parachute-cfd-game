...
# ----------------------------------------------------------------------
# apply_fixlist_0093.sh – Complete self‑healing CFD‑to‑Godot pipeline v93
# v93 : adds mesh retry loop, auto DB dump, explicit polyMesh check,
#       fixes backslash warnings, and ensures idempotent healing.
# ----------------------------------------------------------------------

REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Self‑Healing v93 (complete) ==="
...
# --------------- heal database (with escaped newlines, fixed backslashes) ---------------
...
def fix_backslashes(s):
    # Replace single backslash with double backslash in regex patterns
    # But keep escaped newlines as is – careful
    return s.replace(r'\.', r'\\.')  # only for literal dot in regex
...
PATTERNS = [
    ...
    (r"cannot find file.*blockMeshDict", "Missing system/blockMeshDict",
...
# --------------- pipeline functions (idempotent, with retry loop) ---------------
run_mesh_step() {
    local case_dir="$1"
    ...
    while [ $retry -lt $max_retries ]; do
        ...
        if diagnose_and_heal "$log_file" "$case_dir"; then
            ...
            # Do not increment retry; we will re‑run the same attempt
...
main() {
    ...
    launch_godot_step
    perform_llm_audit
    echo "=== All steps completed successfully (v93) ==="
}
