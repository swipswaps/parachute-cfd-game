...
# ----------------------------------------------------------------------
# apply_fixlist_0005.sh – Fully compliant with prf.fixlist.v6
# - Detects the exact OpenFOAM bashrc path and adds to ~/.bashrc with exact-line match.
# - Skips PKG-001 (OpenFOAM already installed) to avoid hanging sub‑shell.
# - Preserves all v6 determinism/idempotency fixes:
#   * Content guards in TMPL checks (test -s && grep -q)
#   * find | xargs -r for execute bits (handles empty dir)
#   * [ ! -f ] || ! grep for missing file handling
#   * No || true in PKG-002 (explicit error on missing pvpython)
#   * No heredocs (all python3 -c)
...
# ----------------------------------------------------------------------
# Detect installed OpenFOAM bashrc path (exact file, not wildcard)
OF_BASHRC=""
if [[ -f /usr/lib/openfoam/openfoam2512/etc/bashrc ]]; then
    ...
    # Fallback: try to find any openfoam*/etc/bashrc
    OF_BASHRC=$(find /usr/lib/openfoam -name "bashrc" 2>/dev/null | head -1)
fi

if [[ -z "$OF_BASHRC" ]]; then
    echo "⚠️  WARNING: Could not find OpenFOAM bashrc. Environment source step will be skipped."
else
    # ENV-001 – source OpenFOAM env with exact‑line idempotent guard
    run_step "ENV-001 (source_openfoam_env)" \
        "grep -qxF 'source $OF_BASHRC' ~/.bashrc" \
        "grep -qxF 'source $OF_BASHRC' ~/.bashrc || echo 'source $OF_BASHRC' >> ~/.bashrc" \
        "grep -qxF 'source $OF_BASHRC' ~/.bashrc"
fi
...
# No apply, no check – just a placeholder
...
# ----------------------------------------------------------------------
# PY-001 – Python packages (including trimesh)
...
# ----------------------------------------------------------------------
# TEST-001 – verify collada_to_stl with a cube (requires trimesh)
