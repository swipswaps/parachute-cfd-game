...
# ----------------------------------------------------------------------
# apply_fixlist_0073.sh – Complete self-healing CFD-to-Godot pipeline
#
# Accumulated fixes v58 → v73. All changes derived from confirmed evidence.
#
# v73 changes (VTK-SUBDIR-001, HEAL-DB-VTK-001, LLM-AUDIT-INFO):
...
REPO_ROOT="$(pwd)"
echo "=== Parachute CFD Landing Game – Complete Self-Healing v73 ==="
...
# ------------------------------------------------------------
# 2. Google Earth Pro (retained — installed, not in critical path)
# ------------------------------------------------------------
...
run_step "Commit all changes" \
    ...
    "git add -A && git diff --staged --quiet || git commit -m 'fix: v73 VTK subdir extraction, heal DB pattern, LLM audit info'" \
    "git rev-parse HEAD >/dev/null 2>&1 && git diff --quiet HEAD 2>/dev/null && git diff --staged --quiet 2>/dev/null"

llm_audit_info
echo ""
echo "✅ All fixes applied! (v73)"